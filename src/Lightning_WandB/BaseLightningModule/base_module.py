...
class LitSystemPrune(LightningModule):
    ...
    def forward(self, x):
        out = self.model(x)
        print(f"Weight conv in forward {self.model.conv1.weight[0][0]} ")
        print(f"Weight orig conv in forward {self.model.conv1.weight_orig[0][0]} ")
        print(f"Weight orig in forward {self.original_wgts['model.conv1.weight_orig'][0][0]} ")
        return F.log_softmax(out, dim=1)
